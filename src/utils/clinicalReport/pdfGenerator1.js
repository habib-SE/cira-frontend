...
//   return v;
// }
// function asBulletLines(textOrArray) {
//   if (!textOrArray) return "";
//   if (Array.isArray(textOrArray)) return textOrArray.filter(Boolean).map((t) => `• ${t}`).join("\n");
//   const raw = String(textOrArray).trim();
//   if (!raw) return "";
//   if (raw.includes("\n") || raw.startsWith("-") || raw.startsWith("•")) return raw;
//   return `• ${raw}`;
// }
...
//       { label: "Weight", value: patient_identity_baseline?.weight ?? clinicalData?.patientWeight ?? "—" },
...
//     aiAssessmentOverallStability: extractFlatValue(clinicalData, ["aiAssessment", "overallStability"], "overallStability", "Unknown"),
//     aiAssessmentRedFlagSymptoms: extractFlatValue(clinicalData, ["aiAssessment", "redFlagSymptoms"], "redFlagSymptoms", "Unknown"),
...
//       mental_status_changes: { answer: clinicalData?.review_of_systems_traffic_light_view?.mental_status_changes?.answer || "Unknown" },
//     },
...
//     const bullets = toBulletListLines(text);
//     for (const b of bullets) {
//       y = addBulletLinePaged(doc, b, marginX, y, contentW, 5.2, pageW, pageH, topY, bottomY, () => {
//         setFillHex(doc, COLORS.white);
//         doc.rect(0, 0, pageW, pageH, "F");
//         y = drawHeader();
//         doc.setFont("helvetica", "normal");
...
// function makeHasSentence({ subject, value, missingText = "was not specified" }) {
//   const subj = sentenceCap(subject);
//   if (isMissingValue(value)) return ensurePeriod(`${subj} ${missingText}`);
//   return ensurePeriod(`${subj} has ${String(value).trim()}`);
// }
...
//       makeIsSentence({ subject: "The patient’s name", value: toolPatient?.name || patientName, missingText: "was not provided" }),
...
//       makeIsSentence({ subject: "Biological sex", value: toolPatient?.biological_sex || patientGender, missingText: "was not specified" }),
...
//       makeIsSentence({ subject: "BMI", value: bmi ?? toolPatient?.bmi, missingText: "was not calculated" }),
//        makeSentence({ subject: "Primary concern", verb: "was", value: toolChief?.primary_concern, missingText: "was not specified" }),
//       makeSentence({ subject: "Onset", verb: "was", value: toolChief?.onset, missingText: "was not specified" }),
//       makeSentence({ subject: "Duration", verb: "was", value: toolChief?.duration, missingText: "was not specified" }),
//       makeSentence({ subject: "Severity", verb: "was", value: toolChief?.severity, missingText: "was not specified" }),
//       makeSentence({ subject: "Pattern", verb: "was", value: toolChief?.pattern, missingText: "was not specified" }),
//       makeSentence({ subject: "Previous episodes", verb: "were", value: toolChief?.previous_episodes, missingText: "were not specified" }),
//         makeSentence({
//         subject: "Symptom location or body system involved",
//         verb: "was",
//         value: toolHpi?.location || toolHpi?.location_or_system,
//         missingText: "was not specified",
//       }),
//       isMissingValue(assocSymptoms)
//         ? "Associated symptoms were not reported."
//         : `Associated symptoms included ${String(assocSymptoms).trim()}.`,
//       makeSentence({ subject: "Relieving factors", verb: "were", value: toolHpi?.relieving_factors, missingText: "were not specified" }),
//       makeSentence({ subject: "Worsening factors", verb: "were", value: toolHpi?.worsening_factors, missingText: "were not specified" }),
//     ])
//   );

//   // Relevant negatives (sentences)
//   const negatives = [];
...
//   objectiveParts.push(
//     sentenceBulletsBlock("", [
//       isMissingValue(temperature)
//         ? "Temperature was not measured."
//         : `Temperature was ${String(temperature).trim()}.`,
//       makeYesNoSentence({
//         subject: "Fever",
//         value: toolVitals?.reported_fever,
//         yesText: "Fever was reported.",
//         noText: "Fever was not reported.",
//         unknownText: "Fever status was not specified.",
//       }),
//       makeSentence({ subject: "Blood pressure", verb: "was", value: toolVitals?.blood_pressure, missingText: "was not measured" }),
//       makeSentence({
//         subject: "Heart rate",
//         verb: "was",
...
//         missingText: "was not recorded",
//       }),
//         makeSentence({ subject: "Eating and drinking", verb: "was", value: normalizeYesNoUnknown(toolFunc?.eating_drinking_normally), missingText: "was not specified" }),
//       makeSentence({ subject: "Hydration", verb: "was", value: normalizeYesNoUnknown(toolFunc?.hydration), missingText: "was not specified" }),
//       makeSentence({ subject: "Activity level", verb: "was", value: normalizeYesNoUnknown(toolFunc?.activity_level), missingText: "was not specified" }),
//        makeYesNoSentence({
//         subject: "Recent travel",
//         value: toolExposure?.recent_travel,
...
//         value: toolExposure?.sick_contacts,
...
//       }),
//         makeSentence({ subject: "Smoking status", verb: "was", value: normalizeYesNoUnknown(toolLife?.smoking), missingText: "was not specified" }),
//       makeSentence({ subject: "Alcohol use", verb: "was", value: normalizeYesNoUnknown(toolLife?.alcohol_use), missingText: "was not specified" }),
//       makeSentence({ subject: "Recreational drug use", verb: "was", value: normalizeYesNoUnknown(toolLife?.recreational_drugs), missingText: "was not specified" }),
//            makeSentence({ subject: "Chronic illnesses", verb: "were", value: toolMed?.chronic_illnesses, missingText: "were not specified" }),
//       makeSentence({ subject: "Previous surgeries", verb: "were", value: toolMed?.previous_surgeries, missingText: "were not specified" }),
//       makeSentence({ subject: "Current medications", verb: "were", value: toolMed?.current_medications, missingText: "were not specified" }),
//       makeSentence({ subject: "Drug allergies", verb: "were", value: toolMed?.drug_allergies, missingText: "were not specified" }),
//       makeSentence({ subject: "Family history", verb: "was", value: toolMed?.family_history, missingText: "was not specified" }),
//     ])
//   );
...
//       sentenceBulletsBlock("Top clinical considerations", conditions.slice(0, 3).map((c) => {
//         const pct = Math.round(Number(c.percentage || 0));
//         return pct ? `${c.name} was considered with an estimated likelihood of about ${pct}%.` : `${c.name} was considered.`;
//       }))
//     );
//   } else if (chatSummary?.conditions_matching) {
//     assessmentParts.push(
//       sentenceBulletsBlock("Clinical considerations", String(chatSummary.conditions_matching).split("\n").filter(Boolean))
//     );
//   } else {
//     assessmentParts.push("Clinical considerations were not available.");
//   }

//   if (confidence != null) assessmentParts.push(`Assessment confidence was estimated at ${confidence}%.`);
...
//   assessmentParts.push(
//     sentenceBulletsBlock("Clinical status flags", [
...
//   const clinicianNote = toolFinal?.clinical_note_to_physician || toolAI?.clinical_note_to_physician;
//   if (clinicianNote) {
//     assessmentParts.push(sentenceBulletsBlock("Clinical note to physician", [String(clinicianNote).trim()]));
//   }
...
//     // Keep user provided content, but ensure bullets are sentence-like
...
// File: src/utils/clinicalReport/pdfGenerator1.js
import { jsPDF } from "jspdf";
import starsLogo from "../../assets/stars.svg";
...
export const convertChatSummaryToSOAP = (chatSummary = {}, patientInfo = {}) => {
  ...
  objectiveParts.push(
    sentenceBulletsBlock("", [
      isMissingValue(temperature) ? "Temperature was not measured." : `Temperature was ${String(temperature).trim()}.`,
      makeYesNoSentence({
        subject: "Fever",
        value: toolVitals?.reported_fever,
        yesText: "Fever was reported.",
      ...
      makeSentence({
        ...
        verb: "was",
        value: normalizeYesNoUnknown(toolFunc?.eating_drinking_normally),
        missingText: "was not specified",
      }),
      makeSentence({ subject: "Hydration", verb: "was", value: normalizeYesNoUnknown(toolFunc?.hydration), missingText: "was not specified" }),
      makeSentence({
        ...
        verb: "was",
        value: normalizeYesNoUnknown(toolFunc?.activity_level),
        missingText: "was not specified",
      }),
      makeYesNoSentence({
        subject: "Recent travel",
    ...
    assessmentParts.push(
      sentenceBulletsBlock(
        ...
        conditions.slice(0, 3).map((c) => {
          const pct = Math.round(Number(c.percentage || 0));
          return pct
            ? `${c.name} was considered with an estimated likelihood of about ${pct}%.`
            : `${c.name} was considered.`;
        })
      )
    );
  } else if (chatSummary?.conditions_matching) {
