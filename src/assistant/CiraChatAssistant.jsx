...
// 🔍 UPDATED: Main function to process English summary for display
function processEnglishSummaryForDisplay(englishSummary) {
  if (!englishSummary) return "";

  const cleanSummary = extractCleanClinicalSummary(englishSummary);
...
export function parseConditionsAndConfidence(summary) {
  ...
  // First try the English summary format
  ...
  const isJunk = (text) => bannedWords.some((word) => text.toLowerCase().includes(word));
...
function splitOutSelfCare(summary) {
  ...
  const isHeadingLikeOld = (rawLine, n) => {
    const s = String(rawLine || "").trim();
    if (!s) return false;
    if (s.length > 90) return false;
    if (/[.!?]$/.test(s)) return false;
...
const [isProLocked, setIsProLocked] = useState(true); // pro feature lock ON
  ...
  const parsedSummary = englishConsultSummary
    ? {
        conditions: dedupeConditions(summaryStats.conditions || []),
        confidence: summaryStats.confidence,
      }
  ...
  if (userLanguageConsultSummary) {
    ...
    displaySummary = processEnglishSummaryForDisplay(englishConsultSummary);

    // ✅ MULTI-LANGUAGE extraction also works for English
    selfCareText = extractSelfCareText(englishConsultSummary);

    if (!selfCareText) {
      selfCareText =
        "Home care with rest, fluids, and over-the-counter pain relievers is usually enough for most mild illnesses. " +
    ...
    if (!displaySummary || displaySummary.trim().length < 20) {
      displaySummary = extractCleanClinicalSummary(englishConsultSummary);
    }

    if (!displaySummary || !displaySummary.trim()) {
      displaySummary =
        "Thanks for sharing that. I can help—what symptom is bothering you the most right now?";
    }
  }
            ...
            <div className="mt-4 flex flex-col gap-2">
              <button
                ...
                onClick={() => {
                  ...
                  window.location.href = "/pricing"; // or navigate("/pricing")
                }}
