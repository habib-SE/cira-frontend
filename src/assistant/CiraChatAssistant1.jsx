...
// 🔍 ENHANCED: Function to extract self-care from English summary
function extractSelfCareText(englishSummary) {
  if (!englishSummary) return "";
  
  console.log("Extracting self-care from English summary...");
  
  // First try the multilingual extraction
  const { selfCare } = splitOutSelfCareMultilang(englishSummary);
  
  if (selfCare && selfCare.trim().length > 10) {
    console.log("Found self-care via multilingual extractor:", selfCare.substring(0, 100));
    return selfCare;
  }
  
  // If no explicit self-care section found, generate appropriate advice based on symptoms
  const summaryLower = englishSummary.toLowerCase();
  
  // Extract symptoms and conditions to tailor advice
  const hasFever = /fever|102°f|temperature.*102|high temp/i.test(summaryLower);
  const hasCough = /cough/i.test(summaryLower);
  const hasHeadache = /headache/i.test(summaryLower);
  const hasSoreThroat = /sore throat/i.test(summaryLower);
  const hasBodyAches = /body.*ache|muscle.*ache/i.test(summaryLower);
  const hasFatigue = /fatigue|tired|weak/i.test(summaryLower);
  const hasNausea = /nausea/i.test(summaryLower);
  const hasDiarrhea = /diarrhea/i.test(summaryLower);
  
  // Extract mentioned conditions
  const conditionsMatch = englishSummary.match(/TOP\s*3\s*CONDITIONS[\s\S]*?(?=DOCTOR|SELF-CARE|$)/i);
  let mentionedConditions = [];
  if (conditionsMatch) {
    ...
    for (const pattern of conditionPatterns) {
      let match;
      while ((match = pattern.exec(conditionsText)) !== null) {
        if (match[2]) mentionedConditions.push(match[2].trim());
        else if (match[1]) mentionedConditions.push(match[1].trim());
      }
    }
  }
  
  // Check if it's a respiratory infection
  const isRespiratoryInfection = /viral.*respiratory|common cold|flu|influenza|covid/i.test(summaryLower);
  const isUpperRespiratory = /upper.*respiratory|uri/i.test(summaryLower);
  
  console.log("Detected symptoms:", {
    hasFever, hasCough, hasHeadache, hasSoreThroat, 
    hasBodyAches, hasFatigue, isRespiratoryInfection,
    mentionedConditions
  });
  
  // Generate tailored self-care advice based on detected symptoms
  let selfCareAdvice = "";
  
  // If it's a respiratory infection with fever and cough
  if (isRespiratoryInfection || (hasFever && hasCough)) {
    selfCareAdvice = 
      "For fever management: Rest, stay hydrated with water or electrolyte drinks, use acetaminophen or ibuprofen for fever reduction as directed (follow package instructions).\n" +
      "For cough: Stay hydrated, use honey in warm tea (if over 1 year old), consider cough drops or throat lozenges. Use a humidifier to keep air moist.\n" +
      "General care: Get plenty of rest, eat light nutritious foods, avoid smoking and secondhand smoke.\n\n" +
      ...
      "• Severe dehydration (dry mouth, no urine for 8+ hours, dizziness)\n" +
      "• Bluish lips or face";
  }
  // Fever without other specific symptoms
  else if (hasFever) {
    selfCareAdvice =
      "Fever management:\n" +
      ...
      "• Use lukewarm sponge baths if needed (avoid cold water or alcohol rubs)\n\n" +
      "Monitor temperature every 4-6 hours. Seek medical care if:\n" +
      "• Fever is 104°F (40°C) or higher\n" +
      "• Fever lasts more than 3 days\n" +
      "• You have severe headache, stiff neck, confusion, or sensitivity to light\n" +
  ...
  // Cough without fever
    ...
    selfCareAdvice =
      "Cough management:\n" +
      "• Stay well-hydrated with warm liquids like tea with honey (if over 1 year)\n" +
      "• Use a humidifier or take steamy showers\n" +
      "• Try cough drops or hard candy (for adults and older children)\n" +
      "• Avoid irritants like smoke, dust, and strong perfumes\n" +
      "• Elevate your head with extra pillows when sleeping\n" +
      "• Consider over-the-counter cough medicine as directed\n\n" +
  ...
  // Generic self-care if no specific pattern detected
  else {
    selfCareAdvice =
      ...
      "• Avoid spreading illness to others by staying home when possible\n\n" +
      "When to seek medical help:\n" +
      "• Difficulty breathing or shortness of breath\n" +
      "• Chest pain or pressure\n" +
  ...
  // Add a disclaimer
  selfCareAdvice += "\n\n⚠️ Disclaimer: This advice is for informational purposes only and is not a substitute for professional medical advice. Always consult with a healthcare provider for proper diagnosis and treatment.";
  
  console.log("Generated self-care advice:", selfCareAdvice.substring(0, 150) + "...");
            ...
            <div className="w-full max-w-xl">
              <div className="px-4 pt-6 pb-8">
                ...
                {(userLanguageConsultSummary || englishConsultSummary) && (
                  <section className="w-full mt-6 mb-2 relative z-0">
                    <div className="relative bg-white shadow-sm border border-[#E3E3F3] px-5 py-6 overflow-visible">
                        ...
                        <div className="mt-5 border-t border-gray-100 pt-4">
                          ...
                          {selfCareText ? (
                            ...
                            <p className="text-xs text-gray-600 mb-2">
                              ...
                              usually enough for most mild illnesses. If your fever rises, breathing becomes
                              difficult, you have chest pain, fainting, confusion, severe dehydration,
                              or your symptoms last more than a few days or suddenly worsen, contact a doctor or urgent
                              care.
                            </p>
                          )}

                          <p className="text-[11px] text-gray-400">
                            These are rough estimates and do not replace medical advice. Always consult a healthcare professional if
                            you&apos;re worried.
                          </p>
                        </div>
                      </div>

                      {/* Primary actions - FIXED DROPDOWN */}
                      <div className="mt-6 flex flex-col sm:flex-row gap-3 relative">
                        ...
                        <div className="relative flex-1" ref={downloadMenuRef}>
                          <button
                            type="button"
                            onClick={() => setIsDownloadMenuOpen((prev) => !prev)}
                            className="w-full inline-flex items-center justify-between px-4 py-2 sm:py-3 text-sm font-medium rounded-lg bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white transition-colors shadow-md"
                          >
                            <span>Download Reports</span>
                            {/* simple chevron */}
                            <svg
                          ...
                          {isDownloadMenuOpen && (
                            <div
                              className="absolute z-50 mt-2 w-full rounded-xl border border-gray-200 bg-white shadow-2xl text-sm overflow-hidden divide-y divide-gray-100"
                              style={{
                                top: '100%',
                                left: 0,
                                right: 0
                            ...
                            >
                              <button
                                type="button"
                                className="group w-full flex items-center px-4 py-3 hover:bg-purple-50 hover:text-purple-700 active:bg-purple-100 transition-colors text-left"
                                onClick={() => {
                                  setIsDownloadMenuOpen(false);
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    <span className="font-medium group-hover:underline">Doctor Clinical Report</span>
                                    <p className="text-xs text-gray-500">Comprehensive PDF for doctors</p>
                                  </div>
                                </div>
                              </button>
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    <span className="font-medium group-hover:underline">SOAP / EHR Note</span>
                                    <p className="text-xs text-gray-500">Structured medical note format</p>
                                  </div>
                                </div>
                              </button>
                            </div>
                        ...
                        <button
                          type="button"
                          onClick={handleFindDoctorSpecialistClick}
                          className="flex-1 bg-[#E4ECFF] text-[#2F4EBB] rounded-lg text-sm py-2.5 font-medium shadow-sm hover:bg-[#D4E0FF] transition-colors flex items-center justify-center gap-2"
                        >
                      ...
                      {/* ✅ Pro lock overlay - optional */}
                      {isProLocked && false && ( // Set to false to disable for now
                        <div className="absolute inset-0 z-50 flex items-center justify-center">
                          <div className="absolute inset-0 bg-white/80 backdrop-blur-sm" />
                          <div className="relative z-20 w-full max-w-sm mx-6 rounded-2xl border border-gray-200 bg-white shadow-2xl p-5 text-center">
                            {/* ... pro lock content ... */}
                          </div>
                        </div>
                      )}
